...
class PathExecutorNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('lookahead_dist', 0.22)     # ≈ 4 sel grid (~0.056 m/sel)
    ...
    def _handle_path_staleness(self):
        ...
        if not self.path_points:
            if mode == 'go_to_goal' and self.goal_position is not None:
                self.get_logger().warn("No path yet → fallback go-to-goal.")
                return 'go_to_goal'
            elif mode == 'slowdown' and self.goal_position is not None:
                # Boleh juga langsung go_to_goal (lebih berguna daripada diam)
                self.get_logger().warn("No path yet → continue to goal (slow).")
                self._speed_cap_stale = 0.25
                return 'go_to_goal'
            else:
                self.get_logger().warn("No path and no fallback → stop.")
                return 'stop'
        ...
        if age <= timeout + slack:
            ...
            self.get_logger().warn(f"Path stale {age:.2f}s → continue slowly, replan requested.")
            return 'continue'

        if mode == 'go_to_goal' and self.goal_position is not None:
            self.get_logger().warn("Path expired (hard) → fallback go-to-goal.")
            return 'go_to_goal'
        elif mode == 'slowdown':
            self._speed_cap_stale = 0.20
            self.get_logger().warn("Path expired (hard) → continue very slow.")
            return 'continue'
        else:
            self.get_logger().warn("Path expired (hard) → stop.")
            return 'stop'
